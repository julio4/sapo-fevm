...
task("deploy:membershipnft", "Deploy Membership NFT Contract")
    ...
    .setAction(async ({ logs }, { ethers }) => {
        ...
        if (logs) {
            console.info(
                `MembershipNFT contract has been deployed to: ${membershipNFTContract.address}`
            )
        }
...
task("deploy", "Deploy DataDAOExample contract")
    ...
    .setAction(async ({ admins, membershipnftaddress, logs }, { ethers }) => {
        ...
        const dataDAOExampleContract = await dataDAOExample.deploy(
            adminsList,
            membershipnftaddress,
            {
                gasLimit: 1000000000,
        ...
        if (logs) {
            console.info(
                `DataDAOExample contract has been deployed to: ${dataDAOExampleContract.address}`
            )
        }
